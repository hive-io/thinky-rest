...
function isRestify(app) {
  if (app.name === 'restify') return true;
  if (app.hasOwnProperty('router')) {
    try { app.router; return true; } catch (e) {}
  }
...
module.exports = {
  initialize: function(options) {
    ...
    this.isRestify = !!options.isRestify ? options.isRestify : isRestify(this.app);
  ...
  resource: function(options) {
    ...
    var resource = new Resource({
      ...
      sort: options.sort,
      documentWriteValidation: options.documentWriteValidation,
      documentReadValidation: options.documentReadValidation,
      table: options.table,
      isRestify: this.isRestify
    });
