...
var List = function(options) {
  ...
  if (!options.documentReadValidation && options.resource && options.resource.table) {
    this.table = options.resource.table;
  }
...
List.prototype.fetch = function(req, res, context) {
  var query = this.table ? this._r.table(this.table) : this.model,
  ...
  // execute queries
  var self = this;
  return Promise.all([ query.run(), this.table ? countQuery.count().run() : countQuery.count().execute() ])
